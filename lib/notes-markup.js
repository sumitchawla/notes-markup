...
export default {
  ...
  handle_shortcut(event) {
    ...
    if (keyboardEvent.altKey == true) {
      ...
      switch (code) {
        case "KeyZ":
            txt_with_position("Attendees:\nCC:\n\nMeeting Notes: (Sumit DUE "+ get_date_string(1) + ")" + "\nDiscussed: (" + get_date_string(0) + ")\n\nAction Items:\n\n\n\n",true, true, event);
            return;
         case "KeyN":
          return txt_with_position("\t\t-[" +  get_date_string(0) + "]",true, true, event);
         case "KeyX":
          return txt_with_position("[Done-" +  get_date_string(0) + "]",false, false, event);
         case "Digit1":
